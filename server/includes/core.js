module.exports.getCore = function(conf, redisClient, _, _s, cookie, mysqlConn) {

	var TeamieChatServerCore = function() {};

	/* to return userid or -1 given a cookie string */
	TeamieChatServerCore.prototype.getUserId = function(cookieString, callback) {
		var sidToken = null,
			senderId;
		if (cookieString) {
	...
	TeamieChatServerCore.prototype.onSocketConnect = function(client, sio) {
		var core = this;
...
//to notify users' friend that he goes online/offline
		...
		if (!client.handshake.headers.cookie) {
			...
			core.getUserId(client.handshake.headers.cookie, function(userId) {
				if (userId === - 1) {
		...
		client.on('message', function(msg) {
			core.getUserId(client.handshake.headers['cookie'], function(senderId) {
				if (senderId === - 1) {
						...
						if (_.has(msg, "requestType")) {
							switch (msg.requestType) {
									...
									getUserRoster(senderId, function(results) {
										...
										redisClient.mget(redisKeys,function(err,results){
											...
											getUserGroups(senderId, function(results) {
												...
												core.getRecentThreads(senderId,new Date().getTime(),function(results){
													var threadsToRemove =[];
								...
								case REQUEST_TYPE.getRecentThreads:
									if(hasKeys(msg,["time"])){
										core.getRecentThreads(senderId,msg.time,function(results){
											var responseMsg = {
	...
	TeamieChatServerCore.prototype.getRecentThreads = function (userId, time, callback) {
		var redisKey = _s.sprintf(conf.redis.prefix + "user:%d:threads", userId);
		redisClient.zrevrangebyscore([redisKey, time, 0, 'LIMIT', 0, parseInt(conf.maxRecentThreadsPerRequest)],
	...
	return new TeamieChatServerCore();
};